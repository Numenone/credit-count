 ...
 * ramp on top would spend the only free channel restating it.
 */
export function LlmUsageChart({
  ...
  subtitle: string;
  points: UsagePoint[];
  measure: (point: UsagePoint) => number;
        ...
        <>
          ...
          <div className="mt-4 overflow-x-auto">
            <div
              ...
              role="img"
              aria-label={`${title}: ${format(total)} across ${points.length} intervals. The table below has the same figures.`}
            >
              {points.map((point, i) => {
                  ...
                  >
                    ...
                    {hovered === i && (
                      <div className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 w-max max-w-[15rem] -translate-x-1/2 rounded-[var(--radius-sm)] border border-[var(--line)] bg-[var(--surface)] px-2.5 py-2 text-xs shadow-[var(--shadow-md)]">
                        <p className="font-medium">{new Date(point.bucket).toLocaleString("en-GB")}</p>
